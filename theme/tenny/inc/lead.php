 ...
 * or photo. They call te_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Tenny AB.
 *
 * @package Tenny AB
 */
 ...
 * @return string
 */
function te_sample_note() {
	return te_lead( 'ar_riktig' ) ? '' : ' <span class="te-sample">(platshållare)</span>';
}
 ...
 * @return string File name for te_lead_img().
 */
function te_lead_bild( $i ) {
	$bilder = (array) te_lead( 'bilder', array() );
 ...
 * @return string
 */
function te_ring_label() {
	$namn = te_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, te_lead( 'telefon' ) ) : 'Ring ' . te_lead( 'telefon' );
}
