<?php
/**
 * Tenny AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Tenny AB Leads" plugin, so leads survive a theme change.
 *
 * @package Tenny AB
 */
 ...
 * Patterns read these through te_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'tenny', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'tenny' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'tenny' ) ) );

		register_block_pattern_category( 'tenny-sections', array( 'label' => __( 'Tenny AB sections', 'tenny' ) ) );
		register_block_pattern_category( 'tenny-pages', array( 'label' => __( 'Tenny AB page layouts', 'tenny' ) ) );
	}
);
