...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'tenny-leads' ),
					'singular_name' => __( 'Lead', 'tenny-leads' ),
					'edit_item'     => __( 'Förfrågan', 'tenny-leads' ),
					'search_items'  => __( 'Search leads', 'tenny-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'tenny-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'tenny-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'tenny-leads' ),
			'npl_phone'    => __( 'Phone', 'tenny-leads' ),
			'npl_service'  => __( 'Needs', 'tenny-leads' ),
			'npl_timeline' => __( 'When', 'tenny-leads' ),
			'npl_zip'      => __( 'ZIP', 'tenny-leads' ),
			'npl_source'   => __( 'Source', 'tenny-leads' ),
			'npl_status'   => __( 'Status', 'tenny-leads' ),
			'date'         => __( 'Received', 'tenny-leads' ),
		);
	}
);
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'tenny-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'tenny-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'tenny-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'tenny-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'tenny-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'tenny-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'tenny-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'tenny-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'tenny-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'tenny-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'tenny-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
