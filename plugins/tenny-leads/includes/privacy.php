...
add_filter(
	...
	function ( $exporters ) {
		$exporters['tenny-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'tenny-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'tenny-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'tenny-leads',
						'group_label' => __( 'Quote requests', 'tenny-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['tenny-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'tenny-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
